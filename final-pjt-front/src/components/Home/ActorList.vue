<template>
  <div class="mx-3">
    ...
    <v-container fluid>
      <v-row>
        <v-col cols="12" sm="3" :key="actor.id" v-for="actor in actorList">
          <ActorListCard :actor="actor" />
        </v-col>
        <v-col cols="12" class="d-flex justify-center mt-5">
          <v-btn class="mx-2" fab dark small color="error" v-on:click.prevent="previous(currentPage-1)">
            <v-icon dark>
              fas fa-step-backward
            </v-icon>
          </v-btn>
          <v-btn class="mx-2" fab dark small color="error" v-on:click.prevent="next(currentPage+1)">
            <v-icon dark>
              fas fa-step-forward
            </v-icon>
          </v-btn>
